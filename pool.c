...
int psql_pool_init ( PgConnPool* pool, const char* conninfo, const size_t max_connections )
{
	...
	for ( i = 0; i < max_connections; i++ )
	{
		...
		{
			syslog ( LOG_ERR, "Connection to database failed: %s", PQerrorMessage ( pool->conns[i] ) );
...
int psql_pool_destroy ( PgConnPool* pool )
{
	...
	for ( i = 0; i < pool->size; i++ )
	{
		...
		{
			syslog ( LOG_ERR, "Destroying pool connection to thread '%u' which is still in use", ( unsigned int ) pool->avail[i] );
			PQfinish ( pool->conns[i] );
		}
	}
...
PGconn* psql_pool_acquire ( PgConnPool* pool )
{
	...
	for ( ;; )
	{
		...
		if ( res < 0 )
		{
			syslog ( LOG_ERR, "Locking mutex failed for thread '%u': %d", ( unsigned int ) pthread_self (), res );
			return NULL;
		}
		...
		if ( res < 0 )
		{
			syslog ( LOG_ERR, "Error waiting for free condition in thread '%u': %d", ( unsigned int ) pthread_self (), res );
			( void ) pthread_mutex_unlock ( &pool->lock );
			return NULL;
		}
